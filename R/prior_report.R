...
#' @param tau_posterior_plot Optional pre-captured ggplot from
#'   \code{plot_tau_posterior()}, shown when \code{prior} was derived with
#'   \code{map_prior()} (i.e. \code{prior$prior_type == "map"}). If omitted
#'   and \code{prior} is a MAP prior, the report attempts to render it
#'   directly from \code{prior$tau_posterior}.
...
.write_fallback_qmd <- function() {
  ...
  lines <- c(
    ...
    "}",
    ".pm <- function(p) {",
    "  if (identical(p$prior_type,'map'))",
    "    return(paste0('MAP (', .sc(p$n_trials), ' historical trials)'))",
    "  if (!identical(p$dist,'mixture')) return(.sc(p$method))",
    "  dom <- p$components[[which.max(p$weights)]]",
    "  paste0(.sc(dom$method), ' (pooled)')",
    "}",
    ".pe <- function(p) {",
    "  if (identical(p$prior_type,'map')) return('Historical trials (MAP)')",
    ...
    "```",
    "",
    "### Heterogeneity (tau) Posterior",
    "",
    "```{r tau-check}",
    "#| results: asis",
    "if (has_prior && !has_map) {",
    "  cat('*Not applicable -- this prior was not derived via `map_prior()`.*\\n')",
    "}",
    "```",
    "",
    "```{r tau-posterior}",
    "#| fig-cap: 'Posterior distribution of the between-trial heterogeneity parameter tau.'",
    "if (has_map) {",
    "  if (!is.null(pp_tau_plot)&&inherits(pp_tau_plot,'gg')) print(pp_tau_plot)",
    "  else .sp(function() plot_tau_posterior(pr),",
    "           'Tau posterior plot could not be rendered')",
    "}",
    "```",
    "",
    ...
    "```",
    "",
    "```{r conflict-svalue-note}",
    "#| results: asis",
    "if (has_conf && !is.null(cd$s_value)) {",
    "  cat('::: {.callout-note}\\n')",
    "  cat('S-value = -log2(p): an exploratory companion ",
    "to the p-value above, not a thresholded decision rule. S = 4.3 bits ~ ",
    "p = 0.05; S = 6.6 bits ~ p = 0.01.\\n')",
    "  cat(':::  \\n')",
    "}",
    "```",
    "",
